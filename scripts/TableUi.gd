...
const WIN_PROB_CENTER = 0.40  # Środkowa wartość suwaka – punkt odniesienia dla prestiżu
...
func update_controls():
	...
	slider.step = 0.01
	var spread = 0.05  # +/- 5% od centrum
	slider.min_value = WIN_PROB_CENTER - spread
	slider.max_value = WIN_PROB_CENTER + spread
	
	# Jeśli prawdopodobieństwo stołu wychodzi poza zakres, zresetuj do centrum
	if current_table.win_probability < slider.min_value or current_table.win_probability > slider.max_value:
		current_table.win_probability = WIN_PROB_CENTER
		
	slider.value = current_table.win_probability
	
	min_label.text = "%.0f%%" % ((WIN_PROB_CENTER - spread) * 100)
	max_label.text = "%.0f%%" % ((WIN_PROB_CENTER + spread) * 100)
	mid_label.text = "%.0f%%" % (WIN_PROB_CENTER * 100)
	
	_update_details_label()
	
	win_label.text = "Win Chance: %.2f%%" % (current_table.win_probability * 100)
	
	# Oblicz delta prestiżu względem WIN_PROB_CENTER
	# Prawo (wyższe prawdop. dla gracza) → +3 pkt/krok, lewo → -3 pkt/krok
	var change_percent = (current_table.win_probability - WIN_PROB_CENTER)
...
func _get_upgrade_info(upgrade_id: String) -> Dictionary:
	...
	match upgrade_id:
		"speed":     return {"name": "Fast Dealing"    if is_bj else "Fast Spinning",    "price": UpgradeCosts.TABLE_BJ_SPEED     if is_bj else UpgradeCosts.TABLE_RL_SPEED}
		"charisma":  return {"name": "Professional"    if is_bj else "Elegant Croupier", "price": UpgradeCosts.TABLE_BJ_CHARISMA  if is_bj else UpgradeCosts.TABLE_RL_CHARISMA}
		"master":    return {"name": "Senior Dealer"   if is_bj else "Master Croupier",  "price": UpgradeCosts.TABLE_BJ_MASTER    if is_bj else UpgradeCosts.TABLE_RL_MASTER}
		"felt":      return {"name": "Premium Felt"    if is_bj else "Luxury Felt",      "price": UpgradeCosts.TABLE_BJ_FELT      if is_bj else UpgradeCosts.TABLE_RL_FELT}
		"led":       return {"name": "Brass Finish"    if is_bj else "Gold Finish",      "price": UpgradeCosts.TABLE_BJ_LED       if is_bj else UpgradeCosts.TABLE_RL_LED}
		"chip_rack": return {"name": "Wooden Rack"     if is_bj else "Classic Rack",     "price": UpgradeCosts.TABLE_BJ_CHIP_RACK if is_bj else UpgradeCosts.TABLE_RL_CHIP_RACK}
		"vip":       return {"name": "Leather Seats"   if is_bj else "Velvet Seats",     "price": UpgradeCosts.TABLE_BJ_VIP       if is_bj else UpgradeCosts.TABLE_RL_VIP}
		"test1":     return {"name": "Vintage Cards"   if is_bj else "Mahogany Wheel",   "price": UpgradeCosts.TABLE_BJ_VINTAGE   if is_bj else UpgradeCosts.TABLE_RL_MAHOGANY}
		"test2":     return {"name": "Free Snacks"     if is_bj else "Gourmet Snacks",   "price": UpgradeCosts.TABLE_BJ_SNACKS    if is_bj else UpgradeCosts.TABLE_RL_SNACKS}
	return {"name": "Unknown", "price": 999999}
...
func _on_HSlider_value_changed(value):
	...
	# Oblicz delta prestiżu względem WIN_PROB_CENTER (ta sama stała co w update_controls)
	# Prawo (wyższe prawdop. dla gracza) → +3 pkt/krok, lewo → -3 pkt/krok
	var change_percent = (value - WIN_PROB_CENTER)
	var prestige_change = int(change_percent * 100 * 4)
	var sign_str = "+" if prestige_change > 0 else ""
	var color_tag = "[color=#d4af37]" if prestige_change >= 0 else "[color=#cc4444]"
	prestige_delta_label.text = "[right]" + color_tag + sign_str + str(prestige_change) + " ⭐[/color][/right]"
...
func _on_buy_felt():
	...
	if current_table and GameManager.money >= info.price and not current_table.has_felt:
		...
		current_table.add_prestige_bonus(UpgradeCosts.PRESTIGE_TABLE_FELT)
		_update_upgrade_buttons()
		update_header()
...
func _on_buy_led():
	...
	if current_table and GameManager.money >= info.price and not current_table.has_led:
		...
		current_table.add_prestige_bonus(UpgradeCosts.PRESTIGE_TABLE_LED)
		_update_upgrade_buttons()
		update_header()
...
func _on_buy_chip_rack():
	...
	if current_table and GameManager.money >= info.price and not current_table.has_chip_rack:
		...
		current_table.add_prestige_bonus(UpgradeCosts.PRESTIGE_TABLE_CHIP_RACK)
		_update_upgrade_buttons()
		update_header()
...
func _on_buy_vip():
	...
	if current_table and GameManager.money >= info.price and not current_table.has_vip_seats:
		...
		current_table.add_prestige_bonus(UpgradeCosts.PRESTIGE_TABLE_VIP)
		_update_upgrade_buttons()
		update_header()
...
func _on_buy_spinner():
	...
	if current_table and GameManager.money >= info.price and not current_table.has_spinner:
		...
		current_table.add_prestige_bonus(UpgradeCosts.PRESTIGE_TABLE_VINTAGE)
		_update_upgrade_buttons()
		update_header()
...
func _on_buy_drinks():
	...
	if current_table and GameManager.money >= info.price and not current_table.has_drinks:
		...
		current_table.add_prestige_bonus(UpgradeCosts.PRESTIGE_TABLE_SNACKS)
		_update_upgrade_buttons()
		update_header()
...
func _init_prestige_descs() -> void:
	...
	var descs = {
		"CenterContainer/Panel/VBoxContainer/TabContainer/Table Upgrade/ScrollContainer/VBoxContainer/VelvetFeltPanel/MarginContainer/HBoxContainer/VBoxContainer/Desc":      "+%d ⭐ Prestige" % UpgradeCosts.PRESTIGE_TABLE_FELT,
		"CenterContainer/Panel/VBoxContainer/TabContainer/Table Upgrade/ScrollContainer/VBoxContainer/LEDLightingPanel/MarginContainer/HBoxContainer/VBoxContainer/Desc":      "+%d ⭐ Prestige" % UpgradeCosts.PRESTIGE_TABLE_LED,
		"CenterContainer/Panel/VBoxContainer/TabContainer/Table Upgrade/ScrollContainer/VBoxContainer/GoldChipRackPanel/MarginContainer/HBoxContainer/VBoxContainer/Desc":    "+%d ⭐ Prestige" % UpgradeCosts.PRESTIGE_TABLE_CHIP_RACK,
		"CenterContainer/Panel/VBoxContainer/TabContainer/Table Upgrade/ScrollContainer/VBoxContainer/VIPSeatingPanel/MarginContainer/HBoxContainer/VBoxContainer/Desc":     "+%d ⭐ Prestige" % UpgradeCosts.PRESTIGE_TABLE_VIP,
		"CenterContainer/Panel/VBoxContainer/TabContainer/Table Upgrade/ScrollContainer/VBoxContainer/ExtraTestPanel1/MarginContainer/HBoxContainer/VBoxContainer/Desc":     "+%d ⭐ Prestige" % UpgradeCosts.PRESTIGE_TABLE_VINTAGE,
		"CenterContainer/Panel/VBoxContainer/TabContainer/Table Upgrade/ScrollContainer/VBoxContainer/ExtraTestPanel2/MarginContainer/HBoxContainer/VBoxContainer/Desc":     "+%d ⭐ Prestige" % UpgradeCosts.PRESTIGE_TABLE_SNACKS,
	}
	for path in descs:
